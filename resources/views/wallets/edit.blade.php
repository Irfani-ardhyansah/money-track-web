<x-layouts.app :title="'Edit Dompet'">
    ...
    <div class="flex items-center gap-3 px-4 pt-5 pb-4">
        ...
        <h1 class="text-lg font-semibold">Edit Dompet</h1>
    </div>

    <form method="POST" action="{{ route('wallets.update', $wallet) }}" class="px-4 space-y-4">
        @csrf @method('PATCH')
        ...
        <div>
            ...
            <select name="type"
                ...
                @foreach(['cash' => 'Cash / Tunai', 'bank' => 'Rekening Bank', 'e-wallet' => 'Dompet Digital', 'savings' => 'Tabungan', 'investment' => 'Investasi', 'other' => 'Lainnya'] as $val => $label)
                    <option value="{{ $val }}" @selected(old('type', $wallet->type) === $val)>{{ $label }}</option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <select name="parent_id"
                ...
                @foreach($parents as $parent)
                    <option value="{{ $parent->id }}" @selected(old('parent_id', $wallet->parent_id) == $parent->id)>{{ $parent->name }}</option>
                @endforeach
            </select>
