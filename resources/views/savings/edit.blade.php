<x-layouts.app :title="'Edit Penyesuaian Tabungan'">
    ...
    <div class="flex items-center gap-3 px-4 pt-5 pb-3">
        ...
        <h1 class="text-lg font-semibold">Edit Penyesuaian</h1>
    </div>

    <form method="POST" action="{{ route('savings.update', $saving) }}" class="px-4 space-y-5">
        @csrf @method('PUT')

        {{-- Wallet selector (savings & investment only) --}}
        @php
        $initWalletId    = old('wallet_id', $saving->wallet_id);
        $initWalletObj   = $saving->wallet?->load('parent');
        ...
        {{-- Direction toggle --}}
        @php $currentDirection = $saving->amount >= 0 ? 'add' : 'subtract'; @endphp
        <div>
            ...
            <div class="flex rounded-xl overflow-hidden border border-zinc-700">
                <label class="flex-1 cursor-pointer">
                    <input type="radio" name="direction" value="add" class="sr-only peer"
                           @checked(old('direction', $currentDirection) === 'add')>
                ...
                <label class="flex-1 cursor-pointer border-l border-zinc-700">
                    <input type="radio" name="direction" value="subtract" class="sr-only peer"
                           @checked(old('direction', $currentDirection) === 'subtract')>
                    <span class="block text-center text-sm font-medium py-3 transition-colors
        ...
        <div>
            ...
            <div class="relative">
                ...
                <input type="hidden" name="amount" id="amount_hidden"
                       value="{{ old('amount', abs($saving->amount)) }}">
            </div>
        ...
        <div>
            ...
            <input type="date" name="occurred_at"
                   value="{{ old('occurred_at', $saving->occurred_at->toDateString()) }}"
        ...
        <div>
            ...
            <textarea name="notes" rows="2" placeholder="mis. Saldo awal sebelum pakai app…"
                      class="w-full bg-zinc-800 border border-zinc-700 text-zinc-100 text-base rounded-xl px-4 py-3 focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none placeholder:text-zinc-600 resize-none">{{ old('notes', $saving->notes) }}</textarea>
    ...
    <div class="px-4 mt-6 pb-4">
        <form method="POST" action="{{ route('savings.destroy', $saving) }}"
            ...
            @csrf @method('DELETE')
            <button type="submit"
                    class="w-full text-rose-400 hover:text-rose-300 border border-rose-800/60 hover:border-rose-700 text-sm font-medium rounded-xl py-3 transition-colors">
                Hapus Penyesuaian
            </button>
        </form>
    </div>
