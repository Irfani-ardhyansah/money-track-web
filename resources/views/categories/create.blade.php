<x-layouts.app :title="'Tambah Kategori'">
    ...
    <div class="flex items-center gap-3 px-4 pt-5 pb-4">
        ...
        <h1 class="text-lg font-semibold">Tambah Kategori</h1>
    </div>

    <form method="POST" action="{{ route('categories.store') }}" class="px-4 space-y-4">
        @csrf
        ...
        <div>
            ...
            <div class="grid grid-cols-2 gap-2">
                <label class="flex items-center gap-3 bg-zinc-800 border rounded-xl px-4 py-3 cursor-pointer transition-colors
                              {{ old('type', 'expense') === 'income' ? 'border-emerald-500 bg-emerald-900/30' : 'border-zinc-700' }}">
                    <input type="radio" name="type" value="income" class="hidden" {{ old('type', 'expense') === 'income' ? 'checked' : '' }}>
                    <span class="w-3 h-3 rounded-full bg-emerald-400 shrink-0"></span>
                    <span class="text-sm font-medium">Pemasukan</span>
                </label>
                <label class="flex items-center gap-3 bg-zinc-800 border rounded-xl px-4 py-3 cursor-pointer transition-colors
                              {{ old('type', 'expense') === 'expense' ? 'border-rose-500 bg-rose-900/30' : 'border-zinc-700' }}">
                    <input type="radio" name="type" value="expense" class="hidden" {{ old('type', 'expense') === 'expense' ? 'checked' : '' }}>
        ...
        document.querySelectorAll('input[name="type"]').forEach(radio => {
            radio.addEventListener('change', () => {
                document.querySelectorAll('label:has(input[name="type"])').forEach(label => {
                    ...
                    label.className = label.className.replace(/border-(emerald|rose)-500 bg-(emerald|rose)-900\/30/g, 'border-zinc-700');
                    if (input.checked) {
